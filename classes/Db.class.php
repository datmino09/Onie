...
class Db{
		...
		public function selectOne($sql, $arr = array(), $mode = PDO::FETCH_ASSOC)
		{
			$stm = $this->dbh->prepare($sql);
			if (!$stm->execute($arr)) {
				echo "SQL lỗi."; 
			...
			$this->_numRow = $stm->rowCount();
			return $stm->fetch($mode); // Lấy một dòng duy nhất
		}
		...
		public function getLastInsertedId() {
            return $this->dbh->lastInsertId(); // Sử dụng phương thức lastInsertId() của PDO
        }
